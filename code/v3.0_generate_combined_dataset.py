...
def extract_dfsar_polarimetry(
    ...
    # return df
    # ------------------------------------------------------------------
    # ▸ UPDATED IMPLEMENTATION — v3.0 CPR & DOP with SAR polarimetric
    ...
    if dop_path and Path(dop_path).exists():
        ...
        # ── Path B: Full Stokes data unavailable — set NaN ────────────
        log.warning(
            "Pre-calculated DOP GeoTIFF missing. Full Stokes inter-channel phase data "
            "unavailable in GRI rasters; setting DOP_DFSAR to NaN."
        )
        df["DOP_DFSAR"] = np.nan
